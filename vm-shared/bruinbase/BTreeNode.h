...
class BTLeafNode {
  public:
	...
	static const int LEAF_MAX_ENTRIES = (PageFile::PAGE_SIZE - sizeof(PageId)) / LEAF_ENTRY_SIZE;
    ...
    */
	...
	static void convert(char* buffer, int index, int& key, RecordId rid);
	
	// Converts a leaf entry into a char array of length 12
	static void convertToChar(int key, RecordId rid, char* buf);
	
	//initializes the leaf by setting all the entries to empty, as well as the pointer
	//to the next leaf.
	void initialize();
...
//-----------------------------------------------------------------------------
//Important Assumption: whenever a PageFile is passed to BTNonleafNode, it is assumed
//that it has already been opened, and that we can call on it.
