...
class BTreeIndex {
 public:
  ...
  //return error code. if no error, return 0
  RC insertHelper(int key, const RecordId& rid, PageId cpid, int level, PageId& spid, int& spk);
  
  //helper function to create a new root. 
  RC newRoot(int key, PageId lpid, PageId rpid);
  
  //read the metadata to the page file
  RC readMetadata();
  //write the metadata to the page file
  RC writeMetadata();
