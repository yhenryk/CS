...
class MessageReceiver(basic.LineReceiver):
	...
	def processAT(self, data):
		try:
			...
			reactor.connectTCP('localhost', 12780, factory)
			reactor.connectTCP('localhost', 12782, factory)
			reactor.connectTCP('localhost', 12784, factory)
	...
	def processIAMAT(self, data):
		...
		try:
			...
			outmsg = "Error computing time difference or parsing location. Check that your command is in the proper format. \n"
			self.transport.write(outmsg)
			return
