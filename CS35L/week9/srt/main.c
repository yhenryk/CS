...
struct thread_data {
    int thread_id;
    int thread_total;
    double thread_pixel_dx;
    double thread_pixel_dy;
    double thread_subsample_dx;
    double thread_subsample_dy;
    Vec3* thread_camera_pos;
    Vec3* thread_camera_dir;
    Vec3* thread_bg_color;
    scene_t thread_scene;
    float* print;
};

//float print[128][128][3];
...
void *Render(void *threadarg)
{
    ...
    int total = my_data->thread_total;
    scene_t thread_scene = my_data->thread_scene;
    Vec3* thread_camera_pos;
    thread_camera_pos = (my_data->thread_camera_pos);
    Vec3* thread_camera_dir;
    thread_camera_dir = (my_data->thread_camera_dir);
    Vec3* thread_bg_color;
    thread_bg_color = (my_data->thread_bg_color);
    double thread_pixel_dx = my_data->thread_pixel_dx;
    double thread_pixel_dy = my_data->thread_pixel_dy;
    double thread_subsample_dx = my_data->thread_subsample_dx;
    double thread_subsample_dy = my_data->thread_subsample_dy;
    float * thread_print;
    thread_print = my_data->print;
    int widthlimit = width/total;

    for( int px = (tid*(widthlimit)) ; px< ((tid+1)*widthlimit); px++ )
    {
        const double x = thread_pixel_dx * ((double)( px-(width/2) ));
        for( int py= 0; py< height; ++py )
        {
            const double y = thread_pixel_dy * ((double)( py-(height/2) ));
            Vec3 pixel_color;
            ...
            for( int xs=-halfSamples; xs<=halfSamples; ++xs )
            {
                ...
                {
                    double subx = x + ((double)xs)*thread_subsample_dx;
                    double suby = y + ((double)ys)*thread_subsample_dy;

                    /* construct the ray coming out of the camera, through
                     * the screen at (subx,suby)
                     */
                    ray_t pixel_ray;
                    copy( pixel_ray.org, *thread_camera_pos );
                    Vec3 pixel_target;
                    set( pixel_target, subx, suby, z );
                    sub( pixel_ray.dir, pixel_target, *thread_camera_pos );
                    norm( pixel_ray.dir, pixel_ray.dir );

                    Vec3 sample_color;
                    copy( sample_color, *thread_bg_color );
                    /* trace the ray from the camera that
                     * passes through this pixel */
                    trace( &thread_scene, sample_color, &pixel_ray, 0 );
             ...
             */
             ...
             //thread_print[px][py][0]  = scaled_color[0];
             //thread_print[px][py][1] = scaled_color[1];
             //thread_print[px][py][2] = scaled_color[2];
             *(thread_print + 127*px + 127*py + 0) = scaled_color[0];
             *(thread_print + 127*px + 127*py + 1) = scaled_color[1];
             *(thread_print + 127*px + 127*py + 2) = scaled_color[2];
    }
    }
    pthread_exit(NULL);
}
...
main( int argc, char **argv )
{
    ...
    /* for every pixel */
    //create the arg meant to be passed
    ...
    for(int t = 0; t < nthreads; t++)
    {
        ...
        thread_data_array[t].thread_total = nthreads;
        thread_data_array[t].thread_scene = scene;
        thread_data_array[t].thread_camera_pos = &camera_pos;
        thread_data_array[t].thread_camera_dir = &camera_dir;
        thread_data_array[t].thread_bg_color = &bg_color;
        thread_data_array[t].thread_pixel_dx = pixel_dx;
        thread_data_array[t].thread_pixel_dy = pixel_dy;
        thread_data_array[t].thread_subsample_dx = subsample_dx;
        thread_data_array[t].thread_subsample_dy = subsample_dy;
        thread_data_array[t].print = &(print[0][0][0]);
        //printf("In main: creating thread %d\n", t);
        rc = pthread_create(&threads[t], NULL, Render, (void*) &thread_data_array[t]);
        if(rc) {
    ...
    for(int g = 0; g < width; g++)
    {
        ...
        {
            printf("%.0f %.0f %.0f\n", print[g][z][0], print[g][z][1], print[g][z][2]);
        }
        printf("\n");
    }
