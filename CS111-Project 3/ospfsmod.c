 ...
 ****************************************************************************/
//define atoi
//source: http://www.emicrosoftinterview.com/Technical+Questions/Algorithms(CPP)/332.apx
...
static void my_block_helper(uint32_t b, uint32_t buff[3]);
...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		 ...
		 * EXERCISE: Your code here */
				/* Fix me! */
				/* Fix me! */
		uint32_t o = (f_pos-2)* OSPFS_DIRENTRY_SIZE; //points to the next location in the inode to read from.
		uint32_t type; //pass to filldir
		...
		od = (ospfs_direntry_t *) ospfs_inode_data(dir_oi, o);
		...
		//get the inode and check its type
...
allocate_block(void)
{
	/* EXERCISE: Your code here */
	uint32_t cursor;
	void* map = ospfs_block(2);
	cursor = ospfs_super->os_firstinob;
	
	while(bitvector_test(map, cursor) == 0 && cursor < ospfs_super->os_nblocks) {
		cursor++;
	}
	if(cursor == ospfs_super->os_nblocks) {
		//the disk is full
		return 0;
	}
	else {
		bitvector_clear(map, cursor);
	}
	return cursor;
}
...
*/
/*********
my block_helper function
I could not figure out how the isindir etc. functions worked
*********/
static void 
my_block_helper(uint32_t b, uint32_t a[3])
{
	a[0] = -1; a[1] = -1;
	a[2] = -1;
	if(b >= 0 && b < OSPFS_NDIRECT)
		...
		a[1] = 0; a[0] = b - OSPFS_NDIRECT;
	}
	else if( b < OSPFS_MAXFILEBLKS) {
		b = b - (OSPFS_NDIRECT + OSPFS_NINDIRECT);
		a[2] = 0; a[1] = b/OSPFS_NINDIRECT;
		a[0] = b % OSPFS_NINDIRECT;
	}
}
...
add_block(ospfs_inode_t *oi)
{
	...
	/* EXERCISE: Your code here */
	int o[3];
	uint32_t *indir_ptr; //ptr to indirect block ptrs
	uint32_t *dir_ptr; //ptr to direct block ptrs
	
	allocated[0] = 0;
	allocated[1] = 0;
	
	my_block_helper(n, o);
	allocated[0] = allocated[1] = 0;
	...
	if(o[2] == -1 && o[1] == -1) {
		...
		dir_ptr = (uint32_t *) ospfs_block(oi->oi_indirect);
		dir_ptr[o[0]] = allocated[0];
		oi->oi_size += OSPFS_BLKSIZE;
		return 0;
	} else if(o[2] == 0) {
		//need a doubly indirect block
		if(o[1] == 0 && o[0] == 0) {
			//create it
			...
			//we already have an indirect2 block, but need a new indirect block
			allocated[1] = allocate_block();
			if(allocated[1] == 0){
				free_block(allocated[0]);
				return -ENOSPC;
			...
			empty_block(allocated[1]);
			indir_ptr = (uint32_t *) ospfs_block(oi->oi_indirect2);
			indir_ptr[o[1]] = allocated[1];
		}
		indir_ptr = (uint32_t *) ospfs_block(oi->oi_indirect2);
		dir_ptr = (uint32_t *) ospfs_block(indir_ptr[o[1]]);
		dir_ptr[o[0]] = allocated[0];
		oi->oi_size += OSPFS_BLKSIZE;
		return 0;
	}
...
remove_block(ospfs_inode_t *oi)
{
	...
	my_block_helper(n, o);
	if(o[1] == -1 && o[2] == -1){
		if(oi->oi_direct[o[0]] == 0)
		{
			//block pointer doesn't exist
			goto remove_error;
		}
		free_block(oi->oi_direct[o[0]]);
		oi->oi_direct[o[0]] = 0;
	} else if (o[2] == -1 && o[1] == 0) {
		//n is in the indirect block
		if(oi->oi_indirect == 0)
		{
		...
		//n is in the doubly indirect blocks
		if(oi->oi_indirect2 = 0)
			goto remove_error;
		indir_ptr = (uint32_t *) ospfs_block(oi->oi_indirect2);
		if(indir_ptr[o[1]] == 0) 
		...
		if(o[0] == 0) {
			...
			free_block(indir_ptr[o[0]]);
			indir_ptr[o[0]] = 0;
		}
		if(o[0] == 0 && o[1] == 0 )
		{
			//remove the indirect2 block
...
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
	...
	uint32_t new_size_adj;
	ospfs_direntry_t* temp;
	uint32_t quota = -1;
	//read the quota
	temp = find_direntry(oi, ":filequota", 7);
	if(temp)
		quota = atoi(temp->od_name);
	oi->oi_size = ( (oi->oi_size + OSPFS_BLKSIZE - 1) / OSPFS_BLKSIZE) * OSPFS_BLKSIZE;
	new_size_adj = ((new_size + OSPFS_BLKSIZE - 1) / OSPFS_BLKSIZE) * OSPFS_BLKSIZE;
	while(oi->oi_size < new_size && oi->oi_size < quota && quota > 0 ){
		r = add_block(oi);
		if(r < 0)
		{
			new_size_adj = ((old_size + OSPFS_BLKSIZE -1)/OSPFS_BLKSIZE) * OSPFS_BLKSIZE;
			new_size = old_size;
			break;
		}
	}
	while(oi->oi_size > new_size_adj) {
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		uint32_t byte_offset = *f_pos % OSPFS_BLKSIZE;
		...
		if(byte_offset + count - amount > OSPFS_BLKSIZE)
			n = OSPFS_BLKSIZE - byte_offset;
		else
			n = count - amount;
		retval = copy_to_user(buffer, data+byte_offset, n);
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	/* EXERCISE: Your code here */
	if(*f_pos + count > oi->oi_size) { retval = change_size(oi, *f_pos+count); }
	...
	while (amount < count && retval >= 0) {
		...
		if(byte_offset + count - amount > OSPFS_BLKSIZE)
			...
			n = count - amount;
		retval = copy_from_user(data+byte_offset, buffer, n);
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
 ...
 // Replace this line
	...
	while(offset < dir_oi->oi_size) {
		data = (ospfs_direntry_t *) ospfs_inode_data(dir_oi, offset);
		if(data->od_ino == 0) 
		{
			return data;
		}
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	if(!dir)
		return -EIO;
	ospfs_inode_t *dir_oi = ospfs_inode(dir->i_ino), *inode;
	uint32_t entry_ino = 0;
	ospfs_direntry_t* direntry = create_blank_direntry(dir_oi);
	if(IS_ERR(direntry))
	{
		return PTR_ERR(direntry);
	} else if(dst_dentry->d_name.len > OSPFS_MAXNAMELEN -1 ) { 
		return -ENAMETOOLONG;
	} else if(find_direntry(dir_oi, dst_dentry->d_name.name, dst_dentry->d_name.len))
	{
		return -EEXIST;
	}
	inode = ospfs_inode(src_dentry->d_inode->i_ino);
	if(inode)
	{
		inode->oi_nlink++;
	}
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	if(dentry->d_name.len > OSPFS_MAXNAMELEN -1)
		return -ENAMETOOLONG;
	//find empty directory
	data = create_blank_direntry(dir_oi);
	//next find an empty inode
	...
	for (i = 0; i < OSPFS_NDIRECT;i++) { inode_data->oi_direct[i] = 0; }
	data->od_ino = entry_ino;
	strncpy(data->od_name, dentry->d_name.name, dentry->d_name.len);
	data->od_name[dentry->d_name.len] = 0;
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	uint32_t offset = 0;
	ospfs_direntry_t *data = NULL;
	ospfs_symlink_inode_t *inode_data = NULL;
	int is_quota = 0;
	
	//check if filenames are too long.
	if(dentry->d_name.len > OSPFS_MAXNAMELEN - 1){ return -ENAMETOOLONG;}
	if(strlen(symname) > OSPFS_MAXSYMLINKLEN) {return -ENAMETOOLONG;}
	
	//check if file is a quota
	if(!strcmp(dentry->d_name.name, ":filequota")) {is_quota = 1;}
	
	//find the empty direntry to use.
	while(offset < dir_oi->oi_size) {
		data = (ospfs_direntry_t *) ospfs_inode_data(dir_oi, offset);
		if(data->od_ino = 0)
			break;
	...
	//find an empty inode
	while(entry_ino < ospfs_super->os_ninodes) {
		inode_data = (ospfs_symlink_inode_t *) ospfs_inode(entry_ino);
		if(inode_data->oi_nlink == 0 && entry_ino != 1)
			break;
		entry_ino++;
	}
	if(entry_ino == ospfs_super->os_ninodes)
		return -ENOSPC;
	inode_data->oi_size = strlen(symname);
	inode_data->oi_ftype = OSPFS_FTYPE_SYMLINK;
	inode_data->oi_nlink = 1;
	strcpy(inode_data->oi_symlink, symname);
	data->od_ino = entry_ino;
	strncpy(data->od_name, dentry->d_name.name, dentry->d_name.len);
	data->od_name[dentry->d_name.len] = 0;
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	// Exercise: Your code here.
	char* ret_str = oi->oi_symlink;
	/*
	const char *q = strchr((const char*)oi->oi_symlink, '?');
	//conditional symlink
	if(q)
	{
		if(current->user->uid == 0)
			...
			const char *null_p = strchr(q, '\0');
			if(null_p)
				ret_str = null_p + 1;
		}
	}
	*/
	nd_set_link(nd, ret_str);
	return (void*) 0;
}
